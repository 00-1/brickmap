...
- [ ] `bm-platform` surfaces generic normalised touch events (engine-generic; no game dep).
- [ ] `scraped-again` overlay (2 sliders + 4 buttons, dimmed) on the HUD path; renders headless.
- [ ] Touch-router maps to the existing camera/mode/console actions per the table (flight/walk/menu),
      via a **unit-tested pure mapping**; tap-view casts the beam at the hit point.
- [ ] `A` cruise, `B` board/exit/hail, `1` console, `2` map, tap-blocks-in-console all work.
- [ ] CI green (fmt / clippy -D / tests / wasm); boundary intact; golden hash + headless unchanged.
- [ ] On-device feel-tuning noted as the end-of-run / human follow-up (a real phone).
- [ ] Docs: roadmap D9 entry + this checklist ticked.
