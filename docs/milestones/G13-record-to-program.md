...
Core shipped; the pre-highlight split to **G13b** (see below) per Decision-4's escape hatch.
Game-side only (`scraped-again` console + the existing G7 routine model); no engine change.

- **Action memory** lives on `Console`: a per-agent `traces: HashMap<Agent, VecDeque<Block>>`
  capped at `TRACE_CAP = 10`, session-local. Fed by `App::record_manual` at exactly the
  **manual** sites — `dispatch_block` (its sole caller is the manual console click),
  the `T`-collect / left-click-beam / touch-`Beam` / `H`-hail keybinds. Routine/autopilot acts
  call the effect fns directly and never reach these sites, so the trace stays the player's
  hands (tested: per-agent isolation + cap + manual-only).
- **`trace_to_steps` (pure)** is the literal transform: exact blocks in order, the *only*
  change being run-length folding of identical **adjacent** blocks into `Repeat(n)` + one `Do`
  (chunked at `Repeat`'s 1..=9 ceiling, so a 10-run is `repeat(9), do, do` — no loss, no loop
  for non-adjacent). `Console::trace_to_routine(agent)` wraps it in a `continuous` draft
  (Decision 2) named `trace-N` and opens the existing editor.
- **Ticker + action**: a home row `↻ trace → routine [n]  <recent actions>` renders the active
  agent's trace as glyphs (G12) and, picked, builds the draft. The ticker doubles as the
  "filling as you act" discoverability strip. `active_agent` is synced each frame like `now`.
- **Draft = ordinary G7 data**: enables/edits/persists/runs through the existing paths
  unchanged (tested: a recorded `scan, collect` draft ticks to a scan request + a collect act
  credited to the draft).

### Split: G13b — playback pre-highlight (Eager anticipation)
Pre-highlighting the agent's **next world target** one tick ahead needs target *prediction*
(the act's cell/find is resolved in the app's pick/DDA at act time, not pre-computed) plus a
new faint world-marker draw — more than a small addition, and visual (deferred-review). Split
to **G13b** per Decision 4; the high-value on-ramp (trace→routine + ticker) ships now.

## Acceptance checklist

- [x] Rolling per-agent action memory of *manual* actions (~10, session-local); not polluted
      by autopilot/auto-collect. *(tested: cap, per-agent isolation, manual-only sites.)*
- [x] "trace → routine" builds a draft of the **exact** blocks (only run-length `repeat`
      folding; no inference), opened in the existing editor for manual generalization.
- [x] Draft is ordinary G7 data — enables/edits/persists/runs through existing paths; a
      recorded draft reproduces the manual behaviour (interpreter parity tested).
- [x] The trace is visible filling as you act (ticker row, count + recent actions); glyph-
      rendered per G12.
- [~] Pre-highlight of the agent's next target during playback — **split to G13b** (noted
      above; not small + visual).
- [x] Golden voxel-hash + headless render unchanged; CI green; boundary intact; roadmap G13.
