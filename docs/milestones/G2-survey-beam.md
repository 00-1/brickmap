...
> **Status: ✅ landed.** The signature manual verb of **Scraped Again**
> ([`../game-mechanics.md`](../game-mechanics.md) §6) — built after [G1](G1-data-strata-codex.md).
> Delivered the **new engine capability** (`bm-render::overlay` — post-palette, depth-aware,
> non-palettised) + `scraped-again::beam` (cast/fade, collect-along-path, ride rail, cruiser
> board). *(Built as a left-click feature; it becomes the foot **action-block `fire-beam`**
> when the block substrate is retrofitted in [G4](G4-block-substrate.md).)*
