...
- [x] A deterministic minority (~1 in 4, hash-gated) of inscriptions carry stable per-block names
      (`structures::transliterate` over the block's stratum script — `progress::script_for`, the
      `stratum_of` inverse); colossus labels are always name-bearing, biased deep (see As-built #2).
- [x] Collecting a name-bearer (T / beam / auto-collect — all funnel `Collectible.name` through
      `App::discover`) emits `Event::Discover`; the HUD announces **NAME RECOVERED — <name>** for
      ~5 s; dupes yield normally (Discover is idempotent; the Collect still banks).
- [x] Console: undiscovered **absent** (`visible_palette` + `vocabulary` filter on
      `is_discovered`) · discovered-locked **listed dimmed (`·` marker) + stratum-tagged** ·
      discovered+decoded insertable (`is_unlocked` = discovered AND decoded); Tier-0 implicitly
      pre-discovered (starters bypass the set), so the opening + given routines are untouched.
- [x] Discoveries persist: `pg=` **v4** (append-only `Block::code` list; v1–v3 payloads load as
      starter-only; unknown codes skipped leniently).
- [x] Coverage test (all 12 names findable within 2500 units across seeds 1/42/1337, grid +
      monuments), transliteration determinism+collision test, console-state test
      (`discovery_states_absent_dimmed_insertable`); golden voxel-hash unchanged (terrain
      untouched; no golden-image test exists); CI green (fmt / clippy -D / 192 tests / wasm);
      boundary intact (no engine change).
- [x] Roadmap G9 entry + this checklist ticked on `main`.

## As-built (2026-06-11) — assumptions recorded

1. **Starter pre-discovery is implicit**, not seeded state: `is_discovered(b)` = `b.required()
   .is_none() || set.contains(b)`. Old payloads need no migration, the `pg=` set stays small, and
   collecting a starter's name-bearer is just a normal yield (no dupe event) — same observable
   behaviour as seeding, fewer moving parts.
2. **Colossus labels draw from the full *gated* set, biased deep** (`runfoot` ×3 : seek/circle/goto
   ×1) rather than literally Relics/Signals-only: the vocabulary currently has exactly **one**
   Relics/Signals-tier block (`runfoot`), so literal compliance would name every monument
   identically. Deviation recorded here + in the code; revisit when G10 grows the deep vocabulary.
3. **Bare names** (`Block::name()`: scan, collect, beam, decode, spend, goto, drift, seek, circle,
   walk, hail, runfoot) are letters-only so the transliteration stays clean; `fire-beam` → `beam`,
   `run(foot)` → `runfoot`.
4. **Worldgen-version note:** name-bearing cells' text changed, so their `find_id`s differ from
   pre-G9 — an old save's *scanned* mark on such a cell goes stale (the site simply reappears as
   collectable); codex entries keep their own text and are unaffected. Within the E12 worldgen-
   versioning policy.
5. **Legible name-bearers** (script comprehended, G6) display the plain block name instead of a
   lexicon phrase — the name *is* the meaning. (The find id still hashes the original glyphs.)
   The names-readable-on-collect default (Decision 3) is built; the comprehend-first variant was
   not (per the directive).
