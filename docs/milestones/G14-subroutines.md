...
- [ ] `run(routine)` same-agent step; callee runs in place; renders as an ordinary
      glyph-named block in palette/rows (no seams).
- [ ] Insert-time cycle guard + runtime depth cap (failsoft); duplicate-routine action.
- [ ] Nested step groups in `repeat`/`if` bodies, editable in the cursor UI, **one level**.
- [ ] One-implicit-register invariant documented + tested.
- [ ] `run`+groups persist (`pg=` next version, append-only; old payloads load; missing
      callee → no-op); G11 credit pinned (callee) + shown.
- [ ] Golden voxel-hash + headless render unchanged; CI green; boundary intact; roadmap G14.
