...
fn main() {
    ...
    // The Inventory Topics tab (per-topic progress bars) → halves visual-ref.
    let inv_topics = app::render_inventory_topics_ref(&painter, &font);
    write_png(
        &format!("{out_dir}/inventory-topics-brickmap.png"),
        app::REF_W,
        app::REF_H,
        &inv_topics,
    );
    ...
    let home_fresh = app::render_home_fresh_ref(&painter, &font);
    write_png(
        &format!("{out_dir}/home-fresh-brickmap.png"),
        app::REF_W,
        app::REF_H,
        &home_fresh,
    );
    let home_mid = app::render_home_midprogress_ref(&painter, &font);
    write_png(
        &format!("{out_dir}/home-midprogress-brickmap.png"),
        app::REF_W,
        app::REF_H,
        &home_mid,
    );
    println!("wrote home-brickmap + home-fresh + home-midprogress (430x880 visual-ref)");
