...
//!
//! cargo-apk packages a bare `NativeActivity`, so there's no Java/Kotlin of ours to set a theme or
//! call `WindowInsetsController` from — we reach the framework over **JNI** from the native side.
//! Three best-effort steps, each independently guarded:
//! 1. `Window.addFlags(FLAG_FULLSCREEN)` — drops the status bar (works broadly, incl. ≤ API 29).
//! 2. `decorView.setSystemUiVisibility(IMMERSIVE_STICKY | …)` — the legacy nav-bar hide (deprecated
//!    on API 30+ but still effective on many devices).
//! 3. `Window.getInsetsController().hide(systemBars())` + transient-by-swipe — the API 30+ path.
//!
//! Every JNI call is wrapped so a wrong thread / missing method / thrown Java exception **can only
//! no-op** (logged), never panic or abort the process — this is built blind (no NDK locally; the
//! owner device-judges), so a partial result must degrade gracefully rather than crash the app the
//! way the empty-vertex-buffer bug did.
...
/// Best-effort and panic-free — failures are logged and ignored.
pub fn enable() {
    if let Err(e) = apply() {
