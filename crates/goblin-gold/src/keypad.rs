//! A numeric **keypad** — a reusable, data-free UI widget (engine-candidate). Pure geometry
//! plus hit-testing over a pixel box; **no game/topic literals** (honours the no-leakage seam
//! rule, so it sinks cleanly into the engine later). The renderer chooses each key's glyph;
//! it may fall back from `✓`/`⌫` to ASCII if the face lacks them.
