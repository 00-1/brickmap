//! `text2d` — the engine's legible **2-D prose text** service: bake a TrueType face to a
//! grayscale **coverage atlas** (anti-aliased at the target pixel size — the crispest path for
//! small reading text) and lay out **word-wrapped paragraphs**.
//!
//! The engine's other text path, [`crate::text`], is `font8x8` bitmap glyphs billboarded in the
//! world (great for HUD numbers, marginal for prose). `text2d` is the menu/UI/reading path: the
//! game hands in a font face as data; this bakes it CPU-side (atlas bytes + per-glyph metrics) and
//! lays out positioned quads. The GPU side uploads the atlas as an `R8` coverage texture and draws
//! the quads (the same RGBA→texture pattern `bm-render::hud` uses), so at 1:1 the on-screen pixels
//! equal this coverage exactly. Consumed by the engine's 2-D UI surface ([`crate::ui2d`]).
